...
class Device:
    ...
    def _droidcast_alive(self, timeout: float = 5) -> bool:
        """探测当前端口的 DroidCast 是否能正常出图。"""
        try:
            resp = config.droidcast.session.get(
                f"http://127.0.0.1:{config.droidcast.port}/screenshot",
                timeout=timeout,
            )
    ...
    def _wait_droidcast_alive(self, port: int, timeout: float = 10.0) -> bool:
        ...
        while time.time() < deadline:
            if self._droidcast_alive():
                return True
            time.sleep(1)
        return False

    def _launch_droidcast(self, class_path: str, port: int) -> None:
        """以 export 前缀 + nohup 后台形式启动 DroidCast。

        `CLASSPATH=x app_process ...` 前缀形式在部分真机环境（proot 容器内的
        adb 发起的 shell）下实例会假死：端口可连但截图请求永不返回。
        export 形式配合脱离会话的后台启动可稳定工作。
        """
        apk_path = class_path.removeprefix("CLASSPATH=")
        command = (
            f"nohup sh -c 'export CLASSPATH={apk_path};"
            f" exec app_process / com.rayworks.droidcast.Main --port={port}'"
            " </dev/null >/dev/null 2>&1 &"
        )
        self.client.cmd(["shell", command])
    ...
    def start_droidcast(self) -> bool:
        ...
        for attempt in range(1, 4):
            ...
            self._launch_droidcast(class_path, port)
