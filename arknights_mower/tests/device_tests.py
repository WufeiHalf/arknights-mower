...
class TestStartDroidcast(unittest.TestCase):
    ...
    def test_missing_package_installs_then_starts(self):
        ...
        self.assertEqual(len(shell_calls), 1)  # 启动前 pkill 清理旧实例
        self.device.client.process.assert_called_once_with(
            "export CLASSPATH=/data/app/droidcast/base.apk; exec app_process",
            ["/", "com.rayworks.droidcast.Main", "--port=54321"],
        )
    ...
    def test_unresponsive_instance_retries_then_fails(self):
        ...
        self.assertEqual(self.device.client.process.call_count, 3)
        self.assertEqual(
            self.device.client.process.return_value.terminate.call_count, 3
        )
